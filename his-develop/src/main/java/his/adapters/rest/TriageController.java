...
public class TriageController {
    ...
    public ResponseEntity<TriageResponse> registrarTriaje(
        ...
        // Endpoint legado: se mantiene por compatibilidad pero ejecuta el flujo nuevo CU02.
        TriageResponse response = patientFlowService.triage(toPatientTriageRequest(request), emailPersonal);
    ...
    private PatientTriageRequest toPatientTriageRequest(TriageRequest req) {
        PatientTriageRequest mapped = new PatientTriageRequest();
        mapped.setCitaMedicaId(req.getCitaMedicaId());
        mapped.setDpi(req.getDpi());
        mapped.setPresionSistolica(req.getPresionSistolica());
        mapped.setPresionDiastolica(req.getPresionDiastolica());
        mapped.setFrecuenciaCardiaca(req.getFrecuenciaCardiaca());
        mapped.setTemperatura(req.getTemperatura());
        mapped.setSaturacionOxigeno(req.getSaturacionOxigeno());
        mapped.setPesoKg(req.getPesoKg());
        mapped.setTallaCm(req.getTallaCm());
        return mapped;
    }
