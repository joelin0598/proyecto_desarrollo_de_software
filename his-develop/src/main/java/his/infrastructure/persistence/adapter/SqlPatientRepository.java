...
public class SqlPatientRepository implements PatientRepository {
    ...
    @Override
    public Optional<Patient> findByEmailContacto(String emailContacto) {
        if (emailContacto == null || emailContacto.isBlank()) {
            return Optional.empty();
        }
        return patientJpaRepository.findByEmailContactoIgnoreCase(emailContacto.trim())
                .map(PatientMapper::toDomain);
    }
    ...
    public boolean existsByEmailContacto(String emailContacto) {
        ...
        return patientJpaRepository.existsByEmailContactoIgnoreCase(emailContacto.trim());
    }
}
